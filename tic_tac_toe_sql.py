...
try :
    mydb = mysql.connector.connect(host="localhost", user="root", passwd="madforscience3",
                                   auth_plugin="mysql_native_password")
    mycursor = mydb.cursor()
    mycursor.execute("create database if not exists ttt_database")
    mycursor.execute("use ttt_database")
    mycursor.execute(
        "create table if not exists play_data2(sno varchar(25) not null, name varchar(25) not null, password varchar(25) not null, wins varchar(25) not null, loses varchar(25) not null, draws varchar(25) not null)")
    mycursor.execute("select * from play_data2")
    z = 0
    for i in mycursor:
        z = 1
    if z == 0:
        mycursor.execute("insert into play_data2 values('1','madhav','password','0','0','0')")
    mydb.commit()
    data = mycursor.fetchall()
    rows_num = str(mycursor.rowcount)
    mycursor.execute("select * from play_data2")
    users = []
    passwords = []
    for i in mycursor:
        x,username,userpassword,y,z,a = i
...
signup_logo = PhotoImage(file="requirements/51.png")
signup_btn2 = PhotoImage(file="requirements/53.png")
user_pass_pass2 = PhotoImage(file="requirements/54.png")
...
banner_signup = PhotoImage(file="requirements/52.png")
...
def my_self(win):
    ...
    main.overrideredirect(True)
    main.resizable(0, 0)
    mycursor.execute("select * from play_data2 where sno = "+str(index)+" ")
    total_wins = ''
    total_loses = ''
    total_draws = ''
    for i in mycursor:
...
def account_win(win):
    def sign_up():
        def sign_up_final():
            ...
            if u in users:
                ...
                if p == cp and p != '':
                    mycursor.execute("insert into play_data2 values('"+rows_num+"','"+u+"','"+cp+"','0','0','0')")
                    mydb.commit()
                    # credentials.update_cell(len(users) + 1, 1, u)
                    # credentials.update_cell(len(users) + 1, 2, cp)
                    # user_data.update_cell(len(users) + 1, 1, u)
                    # user_data.update_cell(len(users) + 1, 2, str(0))
                    # user_data.update_cell(len(users) + 1, 3, str(0))
                    # user_data.update_cell(len(users) + 1, 4, str(0))
                    main.destroy()
                    messagebox.showinfo("Success", "Sign up success, Now you can login from Account section")
                    change_window_icons(acc_logo, 2, 0, icon_grid)
                else:
        ...
        Label(entry_frame, image=banner_signup, padx=0, pady=0, bd=0).grid(row=0, column=0)
        Button(entry_frame, image=close_button, highlightthickness=0, bd=0,
               command=lambda: [main.destroy(), change_window_icons(acc_logo, 2, 0, icon_grid)]).grid(row=0,
                                                                                                      column=1)
        Label(entry_frame, image=empty, padx=0, pady=0, bd=0).grid(row=1, column=0,columnspan = 2)
        Label(entry_frame, image=grad, padx=0, pady=0, bd=0).grid(row=5, column=0, rowspan=2)

        Label(entry_frame, image=user_pass_pass2, padx=0, pady=0, bd=0).grid(row=2, column=0, rowspan=3)
        entry1 = Entry(entry_frame, text=signup_user, width=12, font=('verdana', 25), fg='black', bg='white')
        entry1.grid(row=2, column=1)

        entry2 = Entry(entry_frame, text=signup_password, width=12, fg='black', font=('verdana', 25), bg='white')
        entry2.grid(row=3, column=1)

        entry3 = Entry(entry_frame, text=confirm_password, width=12, show="*", fg='black', font=('verdana', 25), bg='white')
        entry3.grid(row=4, column=1)

        Button(entry_frame, image=signup_btn2, cursor='hand2', width=200, height=60, highlightthickness=0, bd=0,
               command=lambda: [sign_up_final()]).grid(padx=4, row=5, column=1)
        change_window_icons(acc_logo2, 2, 0, icon_grid)
    ...
    if logged_in :
        ...
        Button(entry_frame, image=signup_logo, cursor='hand2', width=150, height=50, highlightthickness=0, bd=0,
               command=lambda: [main.destroy(),sign_up()]).grid(padx=4, row=5, column=1)
        change_window_icons(acc_logo2, 2, 0, icon_grid)
...
def win_dialogbox(client):
    ...
    if client == 'user':
        ...
        if logged_in:
            mycursor.execute(" update play_data2 set wins = wins+1 where sno = "+str(index)+"")

    elif client == 'draw':
        bg_photo = draw_result
        if logged_in:
            mycursor.execute(" update play_data2 set draws = draws+1 where sno = "+str(index)+"")

    else:
        bg_photo = comp_win_result
        if logged_in:
            mycursor.execute(" update play_data2 set loses = loses+1 where sno = "+str(index)+"")
    win = Toplevel(window)
...
def comp_response2():
    ...
    if len(user_positions_list) > 1 and len(user_positions_list) < 5:
        ...
        for i in user_positions_list:
            comp_moves.remove(i)
        if [2, 1] in comp_moves:
            move = [2, 1]
        else:
            corner_positions = [[1,0],[1,2],[3,0],[3,2]]
            move = choice(corner_positions)
        Label(button_frame, image=symbol_comp, bd=0).grid(row=move[0], column=move[1])
        comp_positions_list.append([move[0], move[1]])
